confIntPaired <-
function(conf.level, ESTIMATE, data, alternative, npNumbers, np.interval, beta, method, ref.pvalue, delta, useStoredCSM) {
  ...
  rootFuncPaired <- function(x) {
    vapply(x, function(x){alpha - pairedCode(data, alternative=alternative, npNumbers=npNumbers, np.interval=np.interval, beta=beta,
                                               method=method, to.plot=FALSE, ref.pvalue=ref.pvalue,
  ...
  # pvalueRoots <- uniroot.all(rootFuncPaired, lower=-0.9999, upper=0.9999, n=100, tol=0.0001)
  ...
  if (alternative == "greater" || ESTIMATE[[1]] >= 0.9999) { UB <- 1
    ...
    while (UB != prevUB) {
      ...
      UBroots <- uniroot.all(rootFuncPaired, lower=lowerVal, upper=0.9999, n=100, tol=0.0001)
  ...
  if (alternative == "less" || ESTIMATE[[1]] <= -0.9999) { LB <- -1
    ...
    while (LB != prevLB) {
      ...
      LBroots <- uniroot.all(rootFuncPaired, lower=-0.9999, upper=lowerVal, n=100)
