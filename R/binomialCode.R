binomialCode <-
function(data,alternative="two.sided",npNumbers=100,beta=.001,interval=FALSE,method="Z-pooled",cond.row=TRUE,to.plot=TRUE,ref.pvalue=TRUE){
...
if(tolower(method) %in% c("csm","csm modified")){
   ...
   while(sum(apply(Tbls, 1, function(x) all(x == data[1,])))==FALSE){
      ...
      add<-matrix(AC[addRow,],length(addRow),2)
      if(tolower(alternative)=="two.sided"){
      ...
      if(alternative=="less" | (alternative=="two.sided" & twosidedLess)){
         AC<-AC[order(AC[,1],-AC[,2]),]
         AC<-AC[!duplicated(AC[,1]),]
         AC<-AC[order(AC[,2],AC[,1]),]
         AC<-AC[!duplicated(AC[,2]),]
      } else {
         AC<-AC[order(AC[,1],AC[,2]),]
         AC<-AC[!duplicated(AC[,1]),]
         AC<-AC[order(AC[,2],-AC[,1]),]
