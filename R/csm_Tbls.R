...
function(data, alternative, npNumbers, int, beta){
  ...
  Tbls <- {}
  if(alternative=="less" || (alternative=="two.sided" && twosidedLess)){Tbls <- matrix(c(0,Ns[2]), 1, 2)
  ...
  if(alternative=="less" || (alternative=="two.sided" && twosidedLess)){
    AC <- matrix(c(0, Tbls[1,2]-1, Tbls[1,1]+1, Ns[2]), 2, 2, byrow=TRUE)
    if(AC[2,1] > Ns[1]){AC <- AC[-2,,drop=FALSE]}
    if(AC[1,2] < 0){AC <- AC[-1,,drop=FALSE]}
  } else {
    AC <- matrix(c(Ns[1], Tbls[1,2]+1, Tbls[1,1]-1, 0), 2, 2, byrow=TRUE)
    if(AC[2,1] < 0){AC <- AC[-2,,drop=FALSE]}
  ...
  #sum(Tbls[,1]==data[1,1] & Tbls[,2]==data[1,2])
